        ...
        if ($user) {
            ...
            if ($user['password'] === $hashedPass) {
                ...
                setcookie(
                    ...
                    $jwt,         // Cookie value
                    [
                        'expires' => $exp,       // Expiry timestamp
                        'path' => '/',           // Root path
                        'domain' => '',          // Domain (leave blank for default domain)
...
$app->post('/admin/addbook', function (Request $request, Response $response, array $args) {
    ...
    if ($jwt) {
    // Proceed with decoding and validation of the token
    ...
    if (!$jwt) {
        return $response->withStatus(401)->withHeader('Content-Type', 'application/json')
            ->write(json_encode(['status' => 'fail', 'message' => 'No token provided.']));
    }
    ...
    try {
        ...
        if ($decoded->role !== 1) {
            return $response->withStatus(403)->withHeader('Content-Type', 'application/json')
                ->write(json_encode(['status' => 'fail', 'message' => 'Forbidden. Admins only.']));
        }
        ...
        if (!$title || !$year || !$authorNames) {
            return $response->withStatus(400)->withHeader('Content-Type', 'application/json')
                ->write(json_encode(['status' => 'fail', 'message' => 'Missing required fields.']));
        }
        ...
        if (isset($_FILES['thumbnail']) && $_FILES['thumbnail']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!move_uploaded_file($_FILES['thumbnail']['tmp_name'], $filepath)) {
                return $response->withStatus(500)->withHeader('Content-Type', 'application/json')
                    ->write(json_encode(['status' => 'fail', 'message' => 'Failed to upload file.']));
            }
        } else {
            return $response->withStatus(400)->withHeader('Content-Type', 'application/json')
                ->write(json_encode(['status' => 'fail', 'message' => 'Thumbnail file is required.']));
        }
        ...
        return $response->withStatus(401)->withHeader('Content-Type', 'application/json')
            ->write(json_encode(['status' => 'fail', 'message' => 'Invalid token.']));
    }
});
